...
This vertical is local generated app behavior. Generated preview/prod deploy rehearsals, EAS builds, provider command plans, and hosted telemetry export remain rehearsals in this slice. Provider execution is explicit only through `agentstack provider inspect/apply`: Convex preview/production inspect runs only `pnpm exec convex env ... list`, Vercel preview/production inspect runs only `pnpm exec vercel env ls <env>` plus `pnpm exec vercel project ls --json`, Vercel preview apply runs only `pnpm exec vercel deploy --target=preview`, and EAS preview/production inspect runs only `pnpm exec eas env:list --environment <env>`.
