...
If any read-only provider inventory call fails, the command exits nonzero with `FAIL provider reconcile preview` and `Reason: live-read-failed`; it still writes no local state, telemetry, provider resources, provider links, local-cloud state, or ledger entries.
