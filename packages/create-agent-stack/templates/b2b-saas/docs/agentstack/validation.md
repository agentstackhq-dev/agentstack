...
Local validation also scans source, docs, and `.env` files for raw secret-like values. Use `agentstack env set` for local validation state and provider secret stores for real credentials.
