...
Use `pnpm run provider:clerk:preview` and `pnpm run provider:clerk:production` to print the bounded Clerk command plan. Provider inspection uses read and diagnostic commands only, such as `pnpm exec clerk doctor --mode agent`, `pnpm exec clerk env pull --mode agent`, and `pnpm exec clerk config pull --mode agent`. Preview proof may use strict JSON env/config pull results for fixed sanitized category presence only after exact apps-list identity is proven; it never prints raw Clerk URLs, webhook endpoints, IDs, env values, tokens, or command output. Provider plans may still render deterministic setup or review steps such as `pnpm exec clerk init -y` and the production `pnpm exec clerk deploy --mode agent` plan item separately; review those steps before running them outside Agentstack.

Use `pnpm run provider:clerk:proof:preview` only after adding a matching planned or active Clerk preview application row to the provider ledger. It is a preview-only proof contract check that may run bounded read-only Clerk diagnostics, never reads local-cloud state, writes nothing, and can emit sanitized exact identity plus partial app-list/config/env live-coherence evidence when strict apps-list and ledger/manifest gates pass. It still refuses readiness until exact drift/live coherence exists.
