...
Use `pnpm run provider:clerk:proof:preview` only after adding a matching planned or active Clerk preview application row to the provider ledger. It is a preview-only proof contract check that may run bounded read-only Clerk diagnostics, never reads local-cloud state, writes nothing, and refuses readiness until exact Clerk application identity and drift parsers exist.
