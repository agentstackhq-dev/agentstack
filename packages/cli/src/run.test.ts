...
describe("runAgentstack", () => {
  ...
  it("rejects old billing-plan generation because M3 billing is config-first", async () => {
    ...
    expect(code).toBe(1);
    expect(output.join("\n")).toContain(
      "Use agentstack.config.ts billing.entitlements and agentstack billing bootstrap."
    );
    await expect(readFile(join(dir, "packages/domain/src/billing-plans/pro.ts"), "utf8")).rejects.toThrow();
  });
