...
- Provider proof: use `provider proof --service <service> --env preview --resource-type <type> --name <name>` only for preview ledger-gated proof checks. It may run bounded read-only inventory, never reads local-cloud state, and always refuses readiness until exact identity and drift parsers exist.
