...
Clerk Billing webhook support is beta. Treat the Clerk provider plan as deterministic command planning for auth, billing webhook, and env command shapes. Use Clerk provider inspect for explicit read-only diagnostics. Clerk apply is unavailable, so review Clerk dashboard billing/webhook settings before relying on automated subscription state.
