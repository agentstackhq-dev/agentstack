...
The preview deploy and mobile build workflows are local-only. They rehearse the Agentstack path and write local-cloud state; they do not deploy to real providers or submit EAS builds. Local provider env resource rehearsal stores redacted/hash-only metadata, never raw environment values. Real Convex, Vercel, EAS, and Clerk adapters will use the same resource contract later.
