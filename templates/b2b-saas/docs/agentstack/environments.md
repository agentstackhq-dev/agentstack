...
Use `pnpm run provider:convex:inspect:preview` or `agentstack provider inspect --service convex --env preview` for explicit Convex read/diagnostic provider interaction.
For explicit preview execution, use `pnpm run provider:convex:apply:preview` or `agentstack provider apply --service convex --env preview`.
...
Use `pnpm run provider:vercel:apply:preview` or `agentstack provider apply --service vercel --env preview` for explicit Vercel preview provider execution. This runs only `pnpm exec vercel deploy --target=preview` through the provider executor. Vercel env add/update/remove commands remain plan-only, and Vercel production apply is unavailable in this slice.
...
Use `pnpm run provider:eas:inspect:preview` or `agentstack provider inspect --service eas --env preview` for explicit read-only EAS provider inspection. This runs only `pnpm exec eas env:list --environment preview` through the provider executor. EAS project initialization, build execution, env create/update/delete execution, production inspect, and all EAS apply paths are unavailable in this slice.
