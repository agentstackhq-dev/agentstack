...
- Use `pnpm run validate:live:preview` only for non-mutating aggregate live provider reads. It prints per-service proof summaries, can surface gated Clerk preview exact identity diagnostics, and refuses readiness while exact drift/live coherence is not proven for every enabled provider.
...
- Use `provider plan` for deterministic provider command plans. Use `pnpm run provider:preview:plan` for the aggregate preview plan; it is plan-only, prints `Readiness: not-claimed`, and must not be treated as live readiness. Use `pnpm run provider:preview:reconcile` for the aggregate preview reconciliation plan artifact; it prints `Evidence: provider-reconciliation-plan`, `Provider execution: none`, `Mutation: none`, `Live state: not-read`, `Local-cloud state: not-read`, `Operations: not-evaluated`, and `Readiness: not-claimed`; it uses local validation plus sanitized ledger summaries only, reads no live provider/local-cloud/provider-link state, writes no telemetry, local-cloud, provider-link, provider resource, or ledger state, and does not claim identity, drift proof, provisioning, adoption/link confirmation, live coherence, or readiness. Use `provider inspect` for explicit Clerk/Convex diagnostics plus Vercel and EAS preview env-list inspection. Use apply only for explicit Convex execution or Vercel preview deploy execution.
