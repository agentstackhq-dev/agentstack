...
driven by the installed `agentstack` package. M3 proved the next SaaS spine: real Clerk Billing webhook delivery to a
Convex HTTP action and a gated `feature.auditLog` web path. M4 clean-machine packaging is locked until the approach is
discussed.
...
| --- | --- |
| `packages/agentstack` | Public package facade, `agentstack` bin, `agentstack/config` export, create command routing |
| `packages/create-agent-stack` | Generator and B2B SaaS template copy/token replacement |
...
- `packages/agentstack/src/bin.ts` routes `agentstack create` and delegates other commands to `@agentstack/cli`.
...
- `packages/create-agent-stack/src/generate.ts` copies the lean template and replaces app/package tokens.
...
| M4 | Next locked packaging proof. Do not start clean-machine packaging until the M4 approach is discussed. |
...
- Public package installation and clean-machine smoke are not validated yet. That is M4 scope.
