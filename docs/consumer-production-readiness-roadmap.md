...
- Preview provider proof-contract requests through `agentstack provider proof --service <clerk|convex|vercel|eas> --env preview --resource-type <type> --name <name>`. The command is preview-only, fail-closed, read-only, and non-mutating; validates locally before unsupported-shape output; checks the requested resource against the manifest before ledger, inventory, or live reads; requires a matching planned or active ledger row; may run bounded read-only inventory/live inspection; reads no local-cloud state; writes no provider, local, telemetry, provider-link, or ledger state; exposes provider-specific identity read-plan metadata for Clerk/Convex/Vercel/EAS; has a separate sanitized `identityCandidates` artifact boundary and `evaluateProviderIdentityCandidateProof` for missing-proof guidance only; can report exact identity for Clerk preview application proof when strict `apps list --json` parsing matches exactly one app row against manifest and ledger comparison context; can report bounded Vercel preview project exact identity only from strict `project ls --json` evidence matched against manifest, ledger, local-link, and preview env-list proof context; keeps remaining Vercel/EAS environment-scope and project-link proof gaps explicit; can report sanitized partial drift evidence for gated Vercel/EAS preview env-list reads, the narrow Clerk preview app-list live-coherence proof path, and strict Clerk preview config/env category presence after exact apps-list identity; and always refuses readiness today while exact drift/live coherence remains unproven.
...
- Provider identity read-plan metadata and sanitized identity candidate artifacts are requirement/candidate evidence, not exact proof. Candidate proof can reduce missing-proof guidance only; it cannot produce `identity=matched`, readiness, live-safe link/adopt confirmation, or provider mutation permission. The current exact exceptions are narrow Clerk preview application `provider proof`, where exact identity is available only from strict `apps list --json` parsing plus manifest and ledger matched comparison evidence, and bounded Vercel preview project `provider proof`, where exact identity is available only from one strict provider-owned `project ls --json` row matched against manifest, ledger, local-link, and preview env-list proof context. For Clerk, `id` is stable-provider-identity evidence only; `provider-resource-id` requires `resourceId` or `resource_id`. For Vercel, `.vercel/project.json` alone is local link-state evidence only.
...
- Provider proof-contract outputs that can move from narrow Clerk/Vercel exact identity slices, candidate-only proof guidance, proof requirements, and partial Vercel/EAS plus Clerk app-list/config/env drift evidence to broader exact proof results only after additional provider-specific identity parsers and exact drift evaluators exist.
