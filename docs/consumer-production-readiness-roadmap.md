...
- Provider identity read-plan metadata and sanitized identity candidate artifacts are requirement/candidate evidence, not exact proof. Candidate proof can reduce missing-proof guidance only; it cannot produce `identity=matched`, readiness, live-safe link/adopt confirmation, or provider mutation permission. The first exception is Clerk preview `provider proof`, where exact identity is available only from strict `apps list --json` parsing plus manifest and ledger matched comparison evidence. For Clerk, `id` is stable-provider-identity evidence only; `provider-resource-id` requires `resourceId` or `resource_id`.
...
- Provider proof-contract outputs that can move from exact-proof unavailability, candidate-only proof guidance, proof requirements, and partial Vercel/EAS and Clerk app-list drift evidence to exact proof results only after provider-specific identity parsers and exact drift evaluators exist.
