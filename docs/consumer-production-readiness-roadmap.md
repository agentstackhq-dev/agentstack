...
- `agentstack validate --quality` orchestration beyond the current `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm build`, and `pnpm test` commands, including Convex checks, generated anchors, theme checks, telemetry schema/redaction checks, secret scanning, and manifest validation.
