...
- Read-only live inspect paths for Clerk, Convex, Vercel preview, and EAS preview, with sanitized partial facts for successful bounded reads and production/broad-read limitations still explicit.
...
3. Provider identity and discovery: provider-specific identity read-plan metadata now exists, but exact identity parsers/read commands, exact drift/live coherence proof, exact live-safe adoption/link confirmation, full discovery/provisioning, and broad production reads are still missing.
...
- Provider identity read-plan metadata is a requirement map, not exact proof. Exact identity remains unavailable until provider-specific read commands/parsers can prove stable identity without emitting provider-shaped or secret-shaped labels.
...
- Provider-specific identity read-plan metadata converted into exact read commands/parsers for Clerk, Convex, Vercel, and EAS.
...
- Provider proof-contract outputs that can move from exact-proof unavailability, proof requirements, and partial Vercel/EAS drift evidence to exact proof results only after provider-specific identity parsers and exact drift evaluators exist.
