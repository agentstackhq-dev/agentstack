...
- Missing: exact provider identity parsers/read commands, drift proof evaluation, exact live readiness proof, complete provider discovery/adoption/provisioning, real SaaS auth/org/billing/webhook runtime, end-to-end generated Convex API usage from web/mobile, functional UI primitives, real OTel export and hosted observability, preview smoke evidence, production gates, hosted control plane, and public installability.
...
| Provider proof contracts | Partial | `packages/adapters/src/provider-proof-contracts.ts` exports a provider-neutral proof-contract layer. `agentstack provider proof --service <clerk\|convex\|vercel\|eas> --env preview --resource-type <type> --name <name>` runs local validation, checks the requested resource against the manifest before ledger/inventory/live reads, requires a matching planned ledger row, may run bounded read-only inventory/live inspection, and reports proof requirements while refusing readiness. It is preview-only, fail-closed, read-only, and performs no provider, local, ledger, or local-cloud mutations. | Needs provider-specific exact identity parsers/read commands and drift proof evaluation before proof contracts can confirm exactness or feed live-safe link/adopt/readiness. |
...
3. Provider identity and discovery: provider-specific exact identity parsers/read commands, drift proof evaluation, exact live-safe adoption/link confirmation, full discovery/provisioning, and broad production reads are still missing.
...
- Drift proof evaluation against manifest expectations and ledger evidence.
...
- Provider proof-contract outputs that can move from proof requirements to exact proof results only after provider-specific parsers and drift evaluators exist.
...
1. Finish provider integration gaps first: provider-specific exact identity parsers/read commands, drift proof evaluation, live-safe link/adopt confirmation, truthful live validation, and bounded provider-specific live-read semantics.
