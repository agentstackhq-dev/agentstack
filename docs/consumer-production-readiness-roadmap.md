...
3. Provider identity and discovery: provider-specific exact identity parsers/read commands, exact drift/live coherence proof, exact live-safe adoption/link confirmation, full discovery/provisioning, and broad production reads are still missing.
...
- Provider proof-contract outputs that can move from proof requirements and partial Vercel/EAS drift evidence to exact proof results only after provider-specific identity parsers and exact drift evaluators exist.
...
1. Finish provider integration gaps first: provider-specific exact identity parsers/read commands, exact drift/live coherence proof, live-safe link/adopt confirmation, truthful live validation, and bounded provider-specific live-read semantics.
