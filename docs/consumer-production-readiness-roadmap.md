...
- `agentstack validate --quality` orchestration beyond the current `pnpm lint`, `pnpm typecheck`, and `pnpm test` commands, including format check, Convex checks, generated anchors, theme checks, telemetry schema/redaction checks, secret scanning, and manifest validation.
