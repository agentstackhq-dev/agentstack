...
| `validate --live` | Truthful refusal | It runs local validation first, then bounded read-only live provider inventory with `Evidence: live-validation`, `Mutation: none`, and per-provider `Evidence: live-read-inventory`. It does not pass Clerk proof exact context in this slice and refuses readiness today with `Readiness: refused` because exact live coherence remains unproven. | Needs provider-specific exact identity proof beyond the narrow Clerk preview proof path plus exact drift/live coherence proof before it can become a readiness pass. |
