...
> pass. Statements about `agentstack.config.json`, `create-agent-stack`, copied generated docs/scripts/skills,
> readiness percentages, or the absence of live billing evidence are historical. Current source of truth is
> `README.md`, `ARCHITECTURE.md`, `docs/README.md`, and the M1-M4 milestone cards.

## Verdict

Agentstack is about 43-45% of the way toward consumer production readiness.

The current repository is a local command-contract and rehearsal prototype with credible generated guidance, local telemetry, provider command planning, aggregate preview/production local reconciliation plan artifacts, explicit aggregate preview/production live-read reconciliation artifacts with per-service proof diagnostics, provider proof contracts for preview/production diagnostics, provider-specific identity read-plan metadata, a sanitized `identityCandidates` artifact boundary, narrow exact identity slices for Clerk preview/production application proof, bounded Vercel preview/production project proof, and bounded EAS preview/production project proof, Clerk preview app-list plus strict config/env partial live-coherence evidence for provider proof only, Vercel preview candidate guidance plus preview/production project proof evidence from structured env-list evidence and local project-link state, Convex preview/production candidate guidance from structured env-list evidence, EAS preview/production candidate guidance plus exact project-info proof diagnostics from structured env-list evidence and local Expo project-link config evidence, a shared sanitized exact identity decision contract, a provider-neutral live-coherence blocker diagnostic that combines exact identity and drift evidence fail-closed, and partial Convex/Vercel/EAS preview plus Vercel/EAS production env-list drift evidence that still refuses readiness until exact drift/live coherence exists, ledger-gated supported mutations, local provider inventory/link/adopt boundaries, and a first non-mutating live validation command that truthfully refuses readiness. It is not yet a consumer-ready production framework. A consumer still cannot install a versioned `agentstack` package, run `agentstack create` without source paths, connect real Clerk/Convex/Vercel/EAS resources, prove live readiness, deploy preview, build mobile, provision production, and operate the app without manual provider coordination.

The consumer-facing truth is:

- Strong: bootstrap generation, manifest-driven command routing, local validation of Agentstack-owned anchors, generated docs/skills, local wide-event telemetry, provider command plans, local and explicit live aggregate reconciliation artifacts, preview proof-contract gating, provider-specific identity read-plan metadata, sanitized candidate artifact boundaries, ledger-gated supported Convex/Vercel mutations, and local provider inventory/link/adopt discipline.
...
| `templates/b2b-saas/` and package-local mirror | Generated app behavior | Generated app is now a lean consumer app rooted in `apps/web`, `apps/mobile`, `apps/convex`, typed `agentstack.config.ts`, package scripts, and ignored `.agentstack/` state. |
...
| --- | --- | --- | --- |
| Single manifest source of truth | Implemented | `agentstack.config.ts` is the active typed config across templates, docs, CLI context, and generated guidance. | Needs production-hardening of schema ergonomics, policy coverage, and package/runtime contracts. |
| Bootstrap generation | Implemented | `agentstack create` copies the lean B2B SaaS template, rewrites app tokens, and validates generated anchors. | Public npm installability and clean-machine smoke evidence are still missing. |
...
| Local validation | Partial | Bare `agentstack validate` checks manifest/env/guidance/theme/source-secret/generated anchors and prints `Evidence: local-structure`. `agentstack generated validate` checks generated anchors and guidance only with `Evidence: generated-boundary`. `agentstack validate --quality` adds configured local package commands, currently `pnpm format:check`, `pnpm generated:check`, `pnpm lint`, `pnpm typecheck`, `pnpm build`, and `pnpm test`, and prints `Evidence: local-quality`. | It does not yet run Convex checks or full runtime validation beyond the configured local quality commands. |
...
- A lean generated app shape with `apps/web`, `apps/mobile`, `apps/convex`, root `AGENTS.md`, `pnpm-workspace.yaml`, and typed `agentstack.config.ts`; provider state, ledgers, evidence, and smoke artifacts belong under ignored `.agentstack/`.
- A broad CLI router and generated package scripts for validation, local dev, local preview sync, live preview up, provider bootstrap/link/deploy/smoke, auth user fixtures, Clerk Billing fixtures/smoke, and evidence checks.
...
- Explicit aggregate live-read reconciliation artifacts through `agentstack provider reconcile --env <preview|production> --plan --source live`. The command runs local validation first, then calls only bounded read-only provider inventory/inspect primitives for enabled providers in manifest order; prints sanitized current status, identity scope, drift, permission, ledger, read-count, command-count, per-service proof diagnostics, and provider-proof next-step summaries; exits nonzero with `Reason: live-read-failed` if any live read fails; writes no telemetry, local-cloud, provider-link, provider resource, or ledger state; and still does not claim exact identity, exact drift/live coherence, provisioning, adoption/link confirmation, mutation permission, or readiness.
- Provider proof-contract requests through `agentstack provider proof --service <clerk|convex|vercel|eas> --env <preview|production> --resource-type <type> --name <name>`. The command is fail-closed, read-only, and non-mutating; validates locally before unsupported-shape output; checks the requested resource against the manifest before ledger, inventory, or live reads; requires a matching planned or active ledger row; may run bounded read-only inventory/live inspection; reads no local-cloud state; writes no provider, local, telemetry, provider-link, or ledger state; exposes provider-specific identity read-plan metadata for Clerk/Convex/Vercel/EAS including production Convex/EAS read diagnostics and EAS `project:info`; has a separate sanitized `identityCandidates` artifact boundary and `evaluateProviderIdentityCandidateProof` for missing-proof guidance only; can report exact identity for Clerk preview/production application proof when strict `apps list --json` parsing matches exactly one app row against manifest and ledger comparison context; can report bounded Vercel preview/production project exact identity only from strict `project ls --json` evidence matched against manifest, ledger, local-link, and requested-environment env-list proof context; can report bounded EAS preview/production project exact identity only from `project:info` evidence matched against manifest, ledger, local Expo project-link, owner, and requested-environment env-list proof context; can report sanitized partial drift evidence for gated Convex/Vercel/EAS preview env-list reads, bounded Vercel/EAS production env-list reads, the narrow Clerk preview app-list live-coherence proof path, and strict Clerk preview config/env category presence after exact apps-list identity; Convex production proof remains `provider-env-read` diagnostic evidence only; Convex partial drift remains diagnostic only and still leaves Convex identity candidate/ambiguous/unavailable with readiness refused; Clerk, Vercel, and EAS production exact identity remains diagnostic only, with Vercel/EAS production exact identity plus partial `env-list-production` drift still live-coherence blocked; every proof path always refuses readiness today while exact drift/live coherence remains unproven.
...
- Provider inventory/link/adopt:
  ...
  - explicit live inventory via `--source live` or `--live` is bounded read-only evidence that calls only existing inspect primitives, prints `Evidence: live-read-inventory`, `Mutation: none`, sanitized partial facts where bounded read access is proven, sanitized candidate identity evidence summaries when candidate labels are available, and sanitized `missing=` proof labels; with matching manifest and planned or active ledger proof context it can surface narrow Clerk/Vercel/EAS exact identity, but still remains neither readiness, adoption, provisioning, reconciliation, nor broad discovery;
...
A consumer path from a versioned `agentstack` package to real production environments still requires:

1. Public package installability: versioned npm packages, clean-machine `agentstack create` smoke tests, generated scripts that do not depend on prototype source paths, and release provenance.
2. Truthful validation: bare `agentstack validate` is local-structure validation; `agentstack generated validate` is generated-boundary validation; `agentstack validate --quality` runs local package quality gates including `pnpm generated:check`; `agentstack validate --cloud` is local-cloud rehearsal; `agentstack validate --live` is bounded read-only provider validation that can surface preview proof summaries, Clerk production application exact identity, Vercel production project exact identity, and EAS production project exact identity where supported, but currently refuses readiness because exact drift/live coherence is not proven for every enabled provider.
3. Provider identity and discovery: provider-specific identity read-plan metadata and sanitized identity candidate artifacts now exist, including conservative Clerk `apps list --json` candidate parsing, bounded Vercel preview/production `project ls --json` exact project proof, Convex/Vercel preview env-list candidate guidance, and EAS preview/production env-list plus local Expo project-link candidate guidance. Exact identity is implemented only for the narrow Clerk preview/production application, bounded Vercel preview/production project, and bounded EAS preview/production project proof contexts, now including standalone live inventory/live link/live adopt diagnostics when matching manifest and planned or active ledger gates exist; broader exact identity parsers/read commands, exact drift/live coherence proof, exact live-safe adoption/link confirmation, full discovery/provisioning, and broad production reads beyond bounded diagnostics are still missing.
...
- Provider proof-contract requests are preview/production diagnostics, fail-closed, and read-only. Exact identity remains limited to Clerk preview/production application proof, bounded Vercel preview/production project proof, and bounded EAS preview/production project proof; Vercel and EAS production can add partial `env-list-production` drift diagnostics, while Convex production proof remains read/candidate evidence. Proof may run bounded read-only inventory/live inspection only after local validation, manifest resource matching, and matching planned ledger evidence pass. It does not read local-cloud state and must not mutate provider resources, local files, provider links, telemetry, or the ledger.
- Provider identity read-plan metadata and sanitized identity candidate artifacts are requirement/candidate evidence, not exact proof. Candidate proof can reduce missing-proof guidance only; it cannot produce `identity=matched`, readiness, live-safe link/adopt confirmation, or provider mutation permission. The current exact exceptions are narrow Clerk preview/production application `provider proof`, where exact identity is available only from strict `apps list --json` parsing plus manifest and ledger matched comparison evidence; bounded Vercel preview/production project `provider proof`, where exact identity is available only from one strict provider-owned `project ls --json` row matched against manifest, ledger, local-link, and requested-environment env-list proof context; and bounded EAS preview/production project `provider proof`, where exact identity is available only from `eas project:info` provider-owned `fullName`/`ID` matched against manifest, ledger, local Expo project-link, owner, and requested-environment env-list proof context. Clerk, Vercel, and EAS production exact identity diagnostics do not prove exact drift/live coherence or readiness. For Clerk, `id` is stable-provider-identity evidence only; `provider-resource-id` requires `resourceId` or `resource_id`. For Vercel, `.vercel/project.json` alone is local link-state evidence only. For EAS, local Expo `extra.eas.projectId` alone is local link-state evidence only.
...
- Provider proof-contract outputs that can move from narrow Clerk/Vercel/EAS exact identity slices, candidate-only proof guidance, proof requirements, and partial Convex/Vercel/EAS plus Clerk app-list/config/env drift evidence to broader exact proof results only after additional provider-specific identity parsers and exact drift evaluators exist.
...
- Executable provider create/provision/update/no-op plans for all four services beyond the current non-mutating lifecycle classifications.
...
- `agentstack validate --quality` orchestration beyond the current `pnpm format:check`, `pnpm generated:check`, `pnpm lint`, `pnpm typecheck`, `pnpm build`, and `pnpm test` commands, including Convex checks, theme checks, telemetry schema/redaction checks, secret scanning, and manifest validation.
