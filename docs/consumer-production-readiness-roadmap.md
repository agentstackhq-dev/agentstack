...
- Provider proof-contract requests through `agentstack provider proof --service <clerk|convex|vercel|eas> --env preview --resource-type <type> --name <name>`, plus Clerk production application diagnostics through `--service clerk --env production --resource-type application` and Vercel production project diagnostics through `--service vercel --env production --resource-type project`. The command is fail-closed, read-only, and non-mutating; validates locally before unsupported-shape output; checks the requested resource against the manifest before ledger, inventory, or live reads; requires a matching planned or active ledger row; may run bounded read-only inventory/live inspection; reads no local-cloud state; writes no provider, local, telemetry, provider-link, or ledger state; exposes provider-specific identity read-plan metadata for Clerk/Convex/Vercel/EAS; has a separate sanitized `identityCandidates` artifact boundary and `evaluateProviderIdentityCandidateProof` for missing-proof guidance only; can report exact identity for Clerk preview/production application proof when strict `apps list --json` parsing matches exactly one app row against manifest and ledger comparison context; can report bounded Vercel preview/production project exact identity only from strict `project ls --json` evidence matched against manifest, ledger, local-link, and requested-environment env-list proof context; keeps remaining Vercel/EAS environment-scope and project-link proof gaps explicit; can report sanitized partial drift evidence for gated Convex/Vercel/EAS preview env-list reads, the narrow Clerk preview app-list live-coherence proof path, and strict Clerk preview config/env category presence after exact apps-list identity; Convex partial drift remains diagnostic only and still leaves Convex identity candidate/ambiguous/unavailable with readiness refused; Clerk and Vercel production exact identity remain diagnostic only with drift unproven/live coherence blocked; and every proof path always refuses readiness today while exact drift/live coherence remains unproven.
