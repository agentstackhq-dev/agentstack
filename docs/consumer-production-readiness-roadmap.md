...
- Provider proof-contract outputs that can move from narrow Clerk/Vercel exact identity slices, candidate-only proof guidance, proof requirements, and partial Vercel/EAS and Clerk app-list drift evidence to broader exact proof results only after additional provider-specific identity parsers and exact drift evaluators exist.
