...
Agentstack now has bootstrap generation, `agentstack.config.json`, broad CLI routing, local env graph rehearsal, structural validation, local package quality validation via `agentstack validate --quality`, generated guidance/skills, local wide-event telemetry, OTLP-shaped local export, provider command plans for Clerk/Convex/Vercel/EAS, aggregate preview provider planning via `agentstack provider plan --env preview --all`, aggregate preview reconciliation plan artifacts via `agentstack provider reconcile --env preview --plan`, provider proof contracts that are preview-scoped except for Clerk production application and Vercel production project exact-identity diagnostics, provider-specific identity read-plan metadata via `getProviderIdentityReadPlan` for Clerk, Convex, Vercel, and EAS, a separate sanitized `exactIdentityProof` artifact boundary that now requires required labels plus sanitized matched comparison evidence, a separate sanitized `identityCandidates` artifact boundary for provider-specific candidate parsers, a shared `evaluateProviderExactIdentityProof` decision contract, a fail-closed `evaluateProviderIdentityProof` that only returns unavailable or ambiguous outside scoped exact provider-proof paths, a provider-neutral `evaluateProviderDriftProof` that can emit sanitized partial drift evidence for gated Convex/Vercel/EAS preview env-list reads, and a provider-neutral `evaluateProviderLiveCoherenceProof` that combines exact identity and drift evidence fail-closed into `Live coherence: blocked` or `Live coherence: unavailable` blocker diagnostics. The real exact identity slices today are narrow Clerk preview/production application provider-proof and bounded Vercel preview/production project provider-proof; they emit only sanitized exact proof after strict provider-owned JSON parsing plus manifest/ledger comparison gates, and Vercel additionally requires local-link comparison. Clerk and Vercel production proof are exact-identity diagnostic only: they have no exact drift/live coherence and still refuse readiness. Production live validation can now surface Clerk production application and Vercel production project exact identity diagnostics when EAS is not part of the enabled production service set; EAS production live validation remains unsupported before executor use. Standalone Vercel production live inventory, direct Vercel production provider inspect, and Vercel production live link/adopt confirmation can now run bounded read-only env-list plus project-list diagnostics, but they remain ambiguous candidate/read evidence and do not claim exact identity or write link/adopt state. Clerk preview may emit partial drift evidence through `clerk-apps-list-preview` or `clerk-config-preview`; Clerk production live validation is exact-identity diagnostic only and still leaves drift unproven/live coherence blocked. Convex, Vercel, and EAS preview env-list reads can emit sanitized partial drift evidence where provider proof/live validation gates pass. Live coherence is a blocker diagnostic, not readiness proof, and partial drift never becomes exact live coherence. These are not exact drift/live coherence, readiness, link/adopt confirmation, or mutation permission. Live inventory, live link/adopt, `validate --live`, Convex, and EAS still remain ambiguous, candidate-only, missing, or refused as appropriate, and candidate proof only reduces missing-proof guidance when sanitized labels are observed. Convex and EAS preview candidate labels are candidate/read evidence only; Convex still has no exact provider identity, no exact drift/live coherence, no readiness proof, no live-safe link/adopt confirmation, and no mutation permission. The Vercel local project-link candidate is local link-state evidence only and cannot create stable provider identity, provider owner identity, provider resource identity, or exact proof by itself. Raw project IDs, org IDs, env values, provider output, provider IDs, project IDs, URLs, tokens, stdout, stderr, command args, app IDs, resource IDs, owner IDs, ledger row IDs, and command output are not printed. It still lacks broad real exact live identity proof, exact drift/live coherence proof, exact live readiness proof for the read-only live validation runner, broad provider discovery/adoption, broad real provider provisioning, a real generated SaaS runtime, real OTel/network/hosted observability, preview deploy/build smoke evidence, production release gates, hosted control-plane state, and public package installability.
...
- Generated package scripts now expose `provider:vercel:proof:production`, and generated AGENTS/docs/skills/test expectations explain that Convex/EAS production proof remains unsupported while Clerk production application and Vercel production project exact-identity diagnostics are available.
...
- EAS production live validation remains unsupported before provider executor use with `Reason: live-validation-unsupported`.
...
- EAS production live inventory remains unsupported before provider executor use.
...
- Updated generated release docs at that checkpoint to state that Clerk inspect, Vercel preview inspect, and EAS preview inspect are read-only. Current docs now include Vercel production inspect as read-only while Vercel production apply remains unavailable.
...
- Kept production live inventory unsupported before executor use at that checkpoint; Vercel production inventory is now a bounded read-only diagnostic, while EAS production remains unsupported before executor use.
...
- Vercel live-read now includes bounded preview/production env-list plus project-list diagnostics for inventory, inspect, proof/live-validation contexts, and live link/adopt confirmation refusals; these remain diagnostic unless an exact proof context can prove identity. EAS production live-read remains unavailable before executor use.
...
- Current real Convex adapter can emit sanitized preview `identityCandidates` with only `provider-environment-scope` from structured preview env-list evidence. Since the later Clerk and Vercel exact parser slices, exact identity exists only for narrow Clerk preview/production application provider-proof and bounded Vercel preview/production project provider-proof paths after strict provider-owned JSON plus manifest/ledger comparison gates; Convex remains candidate/read evidence only, and candidate proof can reduce missing-proof guidance only. It does not produce `identity=matched`, exact readiness, provider mutation, link/adopt confirmation, or production readiness.
- No real provider CLIs were run directly, and no real provider resources were created, mutated, adopted, linked, or deleted.
