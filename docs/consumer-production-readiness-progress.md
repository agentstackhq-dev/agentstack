...
- Standalone live inventory remains diagnostic only: it keeps identity ambiguous unless a gated exact proof context exists through the shared proof contract, writes no local state, writes no telemetry, does not mutate providers, and does not authorize link, adopt, mutation, or readiness.
...
Most recent final verification for the live inventory candidate-evidence reporting slice:

- Behavior verified: `agentstack provider inventory --source live` now prints candidate identity evidence availability, the provider-specific candidate evaluator, and candidate-reduced identity proof missing labels when sanitized candidate read evidence exists.
- Boundary verified: standalone live inventory remains read-only and diagnostic-only. It does not produce exact identity, stable provider identity, exact drift/live coherence, readiness, link/adopt confirmation, mutation permission, telemetry, local-cloud state, provider links, ledger changes, provider mutations, raw env names, raw env values, raw project IDs, or raw provider output.
- TDD evidence: the focused EAS production live-inventory regression first failed because `Candidate identity evidence: available` was absent, then passed after the inventory print boundary reused the candidate evaluator.
- Focused provider-inventory CLI tests passed: `pnpm vitest run packages/cli/src/run.test.ts -t "provider inventory|EAS production live identity facts|Candidate identity"` passed with 1 file, 9 selected tests, and 229 skipped.
