...
Most recent final verification for the exact-first live inventory identity-summary slice:

- Behavior verified: exact standalone live inventory now prints `Exact identity evidence` and `Exact identity evaluator` when exact proof succeeds, and suppresses candidate missing-proof guidance in that exact case. Candidate identity summaries remain available when exact proof is not established.
- Boundary verified: exact-first inventory summaries remain read-only and diagnostic-only. They do not prove exact drift/live coherence, readiness, link/adopt authorization, mutation permission, provisioning, discovery, telemetry, local-cloud state, provider links, ledger changes, provider mutations, raw env names, raw env values, raw project IDs, or raw provider output.
- TDD evidence: the focused Vercel production live-inventory regression first failed because exact inventory still printed candidate missing-proof guidance and no exact identity summary, then passed after the inventory print boundary preferred exact proof summaries.
- Focused provider-inventory CLI tests passed: `pnpm vitest run packages/cli/src/run.test.ts -t "gated exact identity|provider inventory|live inventory"` passed with 1 file, 13 selected tests, and 225 skipped.
- Provider control-plane tests passed: `pnpm vitest run packages/adapters/src/provider-control-plane.test.ts` passed with 20 tests.
...
- Full CLI tests passed: `pnpm vitest run packages/cli/src/run.test.ts` passed with 1 file and 238 tests.
- `pnpm lint` passed.
