...
1. Finish provider integration gaps first:
   - Add sanitized Clerk/Convex command-level partial facts for successful read-only diagnostics/config/env access while preserving ambiguous identity and refusal semantics.
   - Extend aggregate preview provider planning toward typed reconciliation artifacts only after exact identity and drift semantics are designed.
...
Most recent final orchestrator verification for the structurally parsed EAS preview partial-proof slice:
