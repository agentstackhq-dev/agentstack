...
Provider proof work has bounded exact identity diagnostics for Clerk preview/production applications and Vercel preview/production projects, provider-specific identity read-plan metadata for Clerk/Convex/Vercel/EAS, sanitized exact/candidate artifact boundaries, shared exact identity decision contracts, partial preview drift diagnostics for gated Convex/Vercel/EAS env-list reads, and provider-neutral live-coherence blocker diagnostics. Clerk and Vercel production proof remain exact-identity diagnostic only with no exact drift/live coherence or readiness. Convex production provider proof can now run bounded read-only `convex env --prod list` diagnostics after manifest and planned/active ledger gates, and EAS production provider proof can now run bounded read-only production env-list diagnostics after the same proof gates. Standalone Vercel production live inventory, direct Vercel production inspect, and Vercel production live link/adopt confirmation can run bounded read-only env-list plus project-list diagnostics; standalone Convex production inspect, live inventory, live validation, live link/adopt confirmation, and proof can run bounded read-only `convex env --prod list` diagnostics; standalone EAS production inspect, live inventory, live validation, live link/adopt confirmation, and proof can run bounded read-only production env-list diagnostics. These remain ambiguous candidate/read evidence and do not claim exact identity, write link/adopt state, or authorize mutation.
