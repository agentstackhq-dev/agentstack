...
- Provider proof contracts now identify and order the evidence gates for proof requests, and Vercel/EAS preview env-list reads can produce sanitized partial drift evidence. Exact provider identity parsers/read commands and exact drift/live coherence proof are still missing, so `agentstack provider proof ...` refuses readiness by design.
