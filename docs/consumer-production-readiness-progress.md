...
- `agentstack validate --quality` now runs `pnpm lint` before `pnpm typecheck` and `pnpm test`, preserving the local-only/no-provider/no-telemetry boundary and redacted failure output.
- Generated root package scripts now expose `lint` as a concrete quality command, and generated validation docs describe the lint/typecheck/test quality sequence.
...
- CLI and adapter tests now cover Convex production inspect and live inventory as bounded read-only provider-env evidence. Convex production reads emit `provider-env-read` only; they do not emit environment-scope candidates, exact identity, drift/live coherence, readiness, link/adopt confirmation, or mutation permission.
...
- Added `agentstack validate --quality` for truthful local quality validation. It now runs structural validation first and then `pnpm lint`, `pnpm typecheck`, and `pnpm test` through an injected command runner so unit tests never invoke real package commands.
