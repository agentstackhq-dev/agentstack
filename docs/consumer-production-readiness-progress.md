...
- Added the Clerk production application exact-identity diagnostic slice for provider proof.
- Clerk `provider proof --env production --resource-type application` is now allowed only for Clerk, still ledger-gated before executor use, read-only, local-cloud-free, and non-mutating.
- Strict Clerk apps-list exact identity parsing now accepts a matching production app row for production proof while preserving the existing development app expectation for preview proof. Production proof emits sanitized exact identity evidence only, leaves drift unproven/live coherence blocked, and refuses readiness.
- Generated package scripts now expose `provider:clerk:proof:production`, and generated AGENTS/docs/skills/test expectations explain that Convex/Vercel/EAS production proof remains unsupported.
...
- Verification evidence for this slice: `pnpm vitest run packages/adapters/src/clerk.test.ts packages/adapters/src/provider-proof-contracts.test.ts packages/cli/src/run.test.ts -t "production Clerk app|Clerk production|provider proof|proof contract"` passed with 3 files, 50 selected tests, and 225 skipped; `pnpm vitest run packages/adapters/src/clerk.test.ts packages/adapters/src/provider-proof-contracts.test.ts packages/cli/src/run.test.ts packages/create-agent-stack/src/generate.test.ts` passed with 4 files and 285 tests; `pnpm typecheck` passed; `pnpm test` passed with 28 files and 509 tests; `git diff --check` passed; `diff -rq templates/b2b-saas packages/create-agent-stack/templates/b2b-saas` passed; `git diff -- docs/provider-resource-ledger.md` was clean; and repo-root `.agentstack` was absent.
...
- Exact identity remains narrow: Clerk preview/production application proof and bounded Vercel preview project proof only. Partial drift never becomes exact live coherence, and Convex/EAS remain missing or candidate-only for exact proof.
...
1. Finish provider integration gaps first:
   - Implement provider integration gaps that turn diagnostics into proof: exact drift/live coherence, broader exact provider proof beyond narrow Clerk preview/production application and bounded Vercel preview project slices, and live-safe link/adopt/readiness only after proof exists.
   - Implement provider-specific exact identity parsers/read commands that feed the new comparison contract where the provider can prove stable provider-owned identity; Clerk now has a narrow production application exact-identity diagnostic and Vercel has a bounded preview project proof slice, while broader Vercel coverage, Convex/EAS exact identity, and broad production reads remain gaps.
