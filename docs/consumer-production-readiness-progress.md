...
- Preserved the real-provider boundary: current Clerk/Convex/Vercel/EAS adapters still emit no provider-specific exact proof artifacts or parsers, candidate-only and partial paths remain ambiguous/refused, and `validate --live` remains a truthful refusal without exact live readiness proof.
...
- Complete synthetic exact artifacts with labels plus matched comparison evidence still prove exact through shared tests. Current real Clerk/Convex/Vercel/EAS adapters still emit no provider-specific exact proof artifacts or parsers.
