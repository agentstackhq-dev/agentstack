...
Most recent final verification for the standalone live inventory exact-identity diagnostic slice:

- Behavior verified: `agentstack provider inventory --source live` can now surface `identity=matched identity-scope=exact` for narrow Clerk preview/production application and bounded Vercel preview/production project evidence when matching manifest plus planned or active ledger exact proof context and strict provider-owned reads all match.
- Boundary verified: exact inventory identity remains read-only and diagnostic-only. It does not prove exact drift/live coherence, readiness, link/adopt authorization, mutation permission, provisioning, discovery, telemetry, local-cloud state, provider links, ledger changes, provider mutations, raw env names, raw env values, raw project IDs, or raw provider output.
- TDD evidence: the focused Vercel production live-inventory regression first failed because the row stayed `identity=ambiguous identity-scope=partial`, then passed after standalone inventory reused the exact proof-context builders and shared row scope became exact when exact proof succeeds.
- Focused provider-inventory CLI tests passed: `pnpm vitest run packages/cli/src/run.test.ts -t "gated exact identity|provider inventory|live inventory"` passed with 1 file, 12 selected tests, and 226 skipped.
