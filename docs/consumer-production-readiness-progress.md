...
- Added `agentstack validate --live --env <preview|production>` as the first truthful aggregate live validation command. It reuses bounded read-only provider inventory, preserves `live-validation` versus `live-read-inventory` versus provider inspect `live-read` evidence labels, does not pass Clerk proof exact context in this slice, and refuses readiness while exact drift/live coherence remains unproven.
...
1. Finish provider integration gaps first:
   - Implement provider-specific exact identity parsers/read commands that feed the new comparison contract, likely Clerk or Vercel first because their current candidate shapes are closest to stable-provider/link evidence.
