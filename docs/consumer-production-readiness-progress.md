...
Most recent final verification for the live validation candidate-evidence proof-summary slice:

- Behavior verified: non-exact `agentstack validate --live` per-service provider-proof summaries now print candidate identity evidence availability, the candidate evaluator, and candidate-reduced `Identity proof missing` labels when successful read-only candidate evidence can reduce proof blockers.
- Boundary verified: candidate evidence in live validation remains diagnostic only. It does not produce exact identity, stable provider identity, provider-owner identity, provider-resource-id proof, exact drift/live coherence, readiness, link/adopt confirmation, mutation permission, raw project IDs, raw env names, raw env values, or provider output.
- TDD evidence: the focused EAS production live-validation regression first failed because `Candidate identity evidence: available` was absent from provider-proof summaries, then passed after the live-validation summary writer reused the candidate evaluator for non-exact summaries.
- Focused live-validation/provider-proof CLI tests passed: `pnpm vitest run packages/cli/src/run.test.ts -t "live validation|provider proof|EAS project-link"` passed with 1 file, 29 selected tests, and 207 skipped.
