...
Most recent final verification for the provider lifecycle planning slice:

- Behavior verified: `agentstack provider plan --service <service> --env <preview|production>` and aggregate provider plans now print plan-only `Resource`, `Ledger`, `Lifecycle`, and `Lifecycle reason` lines for every expected Clerk, Convex, Vercel, and EAS preview/production resource.
- Boundary verified: lifecycle classifications are local-control-plane command-plan evidence only. They do not prove external provider existence, exact identity, drift/live coherence, readiness, link/adopt authorization, mutation permission, provisioning, discovery, telemetry, local-cloud state, provider links, ledger changes, provider mutations, raw env names, raw env values, raw provider IDs, or raw provider output.
- TDD evidence: the focused provider-control-plane regression first failed because `createProviderLifecyclePlan` did not exist, and the focused CLI regression first failed because aggregate provider plans printed no lifecycle resource lines. Both passed after the shared lifecycle planner and CLI print boundary were added.
- Focused provider-control-plane lifecycle test passed: `pnpm vitest run packages/adapters/src/provider-control-plane.test.ts -t "lifecycle decisions"` passed with 1 selected test and 20 skipped.
- Focused provider-plan CLI tests passed: `pnpm vitest run packages/cli/src/run.test.ts -t "provider plan|lifecycle decisions"` passed with 13 selected tests and 226 skipped.
- Provider control-plane tests passed: `pnpm vitest run packages/adapters/src/provider-control-plane.test.ts` passed with 21 tests.
...
- Full CLI tests passed: `pnpm vitest run packages/cli/src/run.test.ts` passed with 1 file and 239 tests.
- `pnpm lint` passed.
- `pnpm typecheck` passed.
- `pnpm build` passed.
- `pnpm test` passed: 28 files / 534 tests.
