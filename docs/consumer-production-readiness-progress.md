...
- Provider proof contracts now identify and order the evidence gates for proof requests, but exact provider identity parsers/read commands and drift proof evaluation are still missing. `agentstack provider proof ...` therefore refuses readiness by design.
...
1. Finish provider integration gaps first:
   ...
   - Implement drift proof evaluation against manifest expectations and ledger evidence before any inventory or proof row can move beyond ambiguous partial evidence.
...
Most recent final orchestrator verification for the provider proof-contract slice:
